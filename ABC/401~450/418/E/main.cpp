...
// https://qiita.com/hamamu/items/4d081751b69aa3bb3557 より
...
int main() {
    ...
    unordered_map<pair<ll, ll>, ll> cnt;
    unordered_map<pair<ll, ll>, ll> mid;
    ll ans = 0;
    for (ll i = 0; i < N; ++i) {
        for (ll j = i + 1; j < N; ++j) {
            ll dx = X[j] - X[i];
            ...
            ll g = toolbox::math::gcd(dx, dy);
            dx /= abs(g);
            dy /= abs(g);
            // cout << "x1: " << X[i] << ", y1: " << Y[i] << ", x2: " << X[j] << ", y2: " << Y[j] 
            //     << ", dx: " << dx << ", dy: " << dy << endl;
            ll mid_x = X[i] + X[j];
            ll mid_y = Y[i] + Y[j];
            if (cnt.count({dx, dy})) {
