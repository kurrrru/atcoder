...
int main() {
    ...
    toolbox::datastructure::implicit_treap<ll, toolbox::datastructure::monoid_implicit_treap::monoid_range_add_range_sum> treap(1000010, 0);

    ll ans = 0;
    for (int i = 0; i <= N; i++) {
        treap.update(A[i] + 500000, A[i] + 500000 + 1, 1);
        ans += treap.query(0, A[i] + 500000);
    }
    cout << ans << endl;
}
