...
int main() {
    ...
    cin >> N >> Q;
    ll max_u = 0;
    toolbox::datastructure::implicit_treap<ll, toolbox::datastructure::monoid_implicit_treap::monoid_range_add_range_sum> treap(N + 10, 0);
    for (int i = 1; i < N + 1; i++) {
        treap.update(i, i + 1, 1);
    }
    while (Q--) {
        ll x, y;
        cin >> x >> y;
        ...
        ll c = treap.query(max_u + 1, x + 1);
        cout << c << endl;
        treap.update(y, y + 1, c);
        max_u = x;
    }
