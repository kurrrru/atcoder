...
void d0(ll pos, vector<ll> &A, vector<ll> &ans)
{
	ll N = A.size();
	ll cnt_same = 0;
	...
	for (ll i = 2; i < N; i++)
	{
		if (cnt_same < i - 1)
			break;
		ans[i] += toolbox::math::combination_mod(cnt_same, i - 1, MOD, fac, fac_inv);
		if (ans[i] >= MOD)
			ans[i] -= MOD;
	}
}

void dfs(ll pos, ll len, ll d, vector<ll> &A, vector<ll> &ans)
{
	ll N = A.size();
	if (pos == N)
		return ;
...
int main()
{
	...
	for (ll i = 0; i < N; i++)
		cin >> A[i];
	vector<ll> ans(N, 0);
	fac = toolbox::math::factorial_mod(N, MOD);
	fac_inv = toolbox::math::factorial_inv_mod(N, MOD);
	ans[0] = N;
	ans[1] = N * (N - 1) / 2;
	for (ll i = 0; i < N; i++)
	{
