...
int main()
{
	...
	ll A;
	cin >> A;
	toolbox::datastructure::implicit_treap<ll, toolbox::datastructure::monoid_implicit_treap::monoid_range_add_range_sum> treap(1, 1);
	for (ll i = 1; i < N; i++)
	{
		cin >> A;
		treap.insert(A - 1, i + 1);
	}
	for (ll i = 0; i < N; i++)
	{
		cout << treap.query(i, i + 1) << " \n"[i == N - 1];
	}
}
