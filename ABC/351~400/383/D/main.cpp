...
int main()
{
	...
	for (sqrtN = 1; sqrtN * sqrtN < N; sqrtN++)
		;
	vector<ll> primes = toolbox::math::prime_list(sqrtN);
	ll ans = 0;
	for (ll i = 0; i < primes.size(); i++)
	{
		if (primes[i] > 37)
			break;
		if (toolbox::math::pow(primes[i], 8) <= N)
			ans++;
		else
			break;
	}
	for (ll i = 0; i < primes.size(); i++)
	{
		ll left = i, right = primes.size();
		while (right - left > 1)
		{
			ll mid = (left + right) / 2;
			if (!(LLONG_MAX / (toolbox::math::pow(primes[i], 2)) < toolbox::math::pow(primes[mid], 2)) && toolbox::math::pow(primes[i], 2) * toolbox::math::pow(primes[mid], 2) <= N)
				left = mid;
			else
				right = mid;
		}
