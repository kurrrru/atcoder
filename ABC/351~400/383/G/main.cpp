...
int main()
{
	...
	for (ll i = 1; i < X; i++)
	{
		vector<ll> next_dp(N - K * i, 0);
		toolbox::datastructure::implicit_treap<ll, toolbox::datastructure::monoid_implicit_treap::monoid_range_add_range_max> treap(dp);
		for (ll j = 0; j < N - K * i; j++)
		{
			next_dp[j] = treap.query(0, j) + R[j + K * i];
		}
		dp = next_dp;
