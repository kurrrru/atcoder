...
T pow(T base, T exp)
{
	...
	while (exp)
	{
		if (exp & 1)
		{
			if (ret > std::numeric_limits<T>::max() / base)
...
int main()
{
	ll N;
	cin >> N;
	ll w = 64 - __builtin_clzll(N - 1) + 1;
	vector<ll> A(w + 1);
	A[0] = N;
	for (ll i = 1; i <= w; i++)
	{
		ll left = 1, right = A[i - 1] + 1;
		while (right - left > 1)
		{
			ll mid = (left + right) / 2;
			if (toolbox::pow(mid, i - 1) <= N / mid)
				left = mid;
			else
				right = mid;
		}
	...
	for (ll i = 2; i <= w; i++)
	{
		vector<pair<ll, int>> f = toolbox::factorize(i);
