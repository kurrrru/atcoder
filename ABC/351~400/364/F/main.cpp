...
int main()
{
	ll N, Q;
	cin >> N >> Q;
	toolbox::datastructure::implicit_treap<ll, monoid> treap(N, 2e18);
	vector<ll> A(N + 1, 0);
	ll ans = 0;
	while (Q--)
	{
		ll L, R, C;
		cin >> L >> R >> C;
		treap.update(L - 1, R - 1, C);
	cout << treap[0] << " " << treap[1] << " " << treap[2] << endl;
	...
	cout << treap[0] << " " << treap[1] << " " << treap[2] << endl;
	cout << treap.query(0, 1) << endl;
	cout << treap.query(0, 2) << endl;
	cout << ans + treap.query(0, N - 1) << endl;
}
