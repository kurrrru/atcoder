...
int main()
{
	ll N, Q;
	cin >> N >> Q;
	toolbox::datastructure::implicit_treap<ll, toolbox::datastructure::monoid_implicit_treap::monoid_range_update_range_max> treap(N, 0);
	while (Q--)
	{
		ll op;
		cin >> op;
		if (op == 1)
		{
			ll pos, x;
			cin >> pos >> x;
			treap.update(pos - 1, pos, x);
		}
		else
		{
			ll l, r;
			cin >> l >> r;
			cout << treap.query(l - 1, r - 1) << endl;
		}
	}
}
