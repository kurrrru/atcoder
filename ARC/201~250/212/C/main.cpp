...
int main() {
    ...
    auto fact = factorial_mod(N + M, mod);
    auto fact_inv = factorial_inv_mod(N + M, mod);

    ll ans = 0;
    for (ll k = M; k <= N; k++) {
        if ((N - k) % 2 != 0)
            continue;
        ans += combination_mod((N - k) / 2 + M - 1, M - 1, mod, fact, fact_inv)
            * combination_mod(k + M - 1, 2 * M - 1, mod, fact, fact_inv)
            % mod
            * pow_mod(2, M, mod)
            % mod;
        ans %= mod;
