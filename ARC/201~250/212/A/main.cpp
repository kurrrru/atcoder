...
int main() {
    ...
    auto fact = toolbox::math::factorial_mod(K, mod);
    auto fact_inv = toolbox::math::factorial_inv_mod(K, mod);

    ll ans = toolbox::math::combination_mod(K - 1, 5, mod, fact, fact_inv) * K;
    ...
    for (ll a = 2; a < K; a++) {
        for (ll b = 2; b < K - a; b++) {
            ll c = K - a - b;
            minu += max({a,b,c}) * (a-1) * (b-1) * (c-1);
            minu %= mod;
        }
    }
    // cout << ans << " " << minu << endl;

    ans = (ans + mod - minu) % mod;
    cout << ans << endl;
